...
def modulo_items(family):
    new_family = []
    for i in range(len(family)):
        items, *rest= family[i]
        new_items = list(map(lambda x: [x for _ in range(len(family))], items))
        new_family.append([new_items, *rest])
    return new_family


def doit(family):
    inspects = [0 for _ in range(len(family))]
    family = modulo_items(family)
    for _ in range(10000):
        for i in range(len(family)):
            items, op_func, _, true_monkey, false_monkey = family[i]
            for item in items:
                inspects[i] += 1
                new_items = [op_func(item[m]) % family[m][2] for m in range(len(family))]
                if new_items[i] == 0:
                    family[true_monkey][0].append(new_items)
                else:
                    family[false_monkey][0].append(new_items)
            family[i][0] = []
